"""Fail-closed SQLite runtime contract for the canonical ActaKit store."""
...
class RuntimeContractError(RuntimeError):
    """The loaded SQLite library is not a certified ActaKit runtime."""
...
def verify_runtime_contract() -> None:
    ...
    through ActaKit's durability/projection proofs before it enters the registry.
    """

    version = _version_tuple()
    if version < TARGET_VERSION:
        raise RuntimeContractError(
            f"SQLite {sqlite3.sqlite_version} is below the ActaKit floor 3.53.4"
        )
