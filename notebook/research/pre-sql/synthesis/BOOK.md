...
The source horizon now covers the pending pre-SQL decisions from multiple directions: formal standards, long-running civic/investigative systems, operator document tools, archival/preservation practice, and SQLite itself. All 29 Source Books were individually deep-audited against primary/normative evidence, implementation or real-use evidence where applicable, scars/errata/evolution, source-local claims, bounded transfers, explicit do-not-copy boundaries, schema pressure, and residual risk. The expanded source-book ledgers contain 99 source records and 109 source-local research claims. The schema candidate critical review reopened only the SQLite Book narrowly after executable SQLite semantics exposed storage/query scars that could change the candidate: logical deletion/FTS removal is not physical purge, optional link fields cannot hide inside a STRICT composite primary key, and ClaimRelation traversal must distinguish deduplicated node reachability from parallel edge/path enumeration. The research horizon was extended rather than silently treating those implementation facts as design intuition.
