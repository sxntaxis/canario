"""Open and bootstrap the canonical Canario SQLite database."""
...
SCHEMA_VERSION = 1
MIGRATION_0001_SHA256 = "5226c873487d9bd05fc62b7a1f323d6e804b003cc4e08bd2fe2b531adb6057bb"
...
class DatabaseIdentityError(RuntimeError):
    """A SQLite file is not a valid authority for the requested Canario schema."""
...
def _assert_schema_v1(con: sqlite3.Connection, *, full_integrity: bool) -> None:
    ...
    if application_id != APPLICATION_ID or user_version != SCHEMA_VERSION:
        raise DatabaseIdentityError(
            "not an Canario schema-v1 authority: "
            f"application_id={application_id}, user_version={user_version}"
        )
    ...
    if fts_tables != 3:
        raise DatabaseIdentityError(
            f"Canario schema-v1 inventory mismatch: fts_tables={fts_tables}"
        )
    ...
    for table in ("claim_fts", "representation_fts", "document_fts"):
        ...
        if row != (1,):
            raise DatabaseIdentityError(
                f"Canario FTS secure-delete contract failed for {table}: {row!r}"
            )
    ...
    if full_integrity:
        ...
        if fk_errors:
            raise DatabaseIdentityError(
                f"Canario foreign-key integrity check failed: {fk_errors[:3]!r}"
            )
        integrity = con.execute("PRAGMA integrity_check").fetchone()[0]
        if integrity != "ok":
            raise DatabaseIdentityError(
                f"Canario SQLite integrity check failed: {integrity!r}"
            )
...
def open_writable_v1(path: str | Path) -> sqlite3.Connection:
    """Open an existing Canario schema-v1 authority for canonical writes."""
